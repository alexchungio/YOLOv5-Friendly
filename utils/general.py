...
DATASETS_DIR = Path(os.getenv('YOLOv5_DATASETS_DIR', ROOT.parent / 'dataset'))  # global dataset directory
...
def check_dataset(data, autodownload=True):
    ...
    for k in 'train', 'val', 'names':
        assert k in data, emojis(f"dataset.yaml '{k}:' field missing ❌")
...
def check_amp(model):
    ...
    f = ROOT / 'dataset' / 'images' / 'bus.jpg'  # image to check
...
def yaml_load(file='dataset.yaml'):
...
def download(url, dir='.', unzip=True, delete=True, curl=False, threads=1, retry=3):
    # Multithreaded file download and unzip function, used in dataset.yaml for autodownload
